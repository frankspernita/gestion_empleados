    ...
    <!-- Begin Page Content -->
      ...
      <!-- Page Heading -->
          ...
          <?php while($row = mysqli_fetch_array($datos)){
              $idlocal_prod = $row['idlocal_prod'];
              $idproveedor = $row['idproveedor'];
              $nombre = $row['prod_nombre'];
              $stocka = $row['prod_stockactual'];
              $stockr = $row['prod_stockrecibir'];
              $umedida = $row['prod_umedida'];
              $idvencimiento[] = $row['idvencimiento'];
              $vto_lote[] = $row['vto_lote'];
              $vto_fecha[] = $row['vto_fecha'];
          } ?>
            <div class="form-group">
                <div class="form-row">
                    <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                        <div class="selector-proveedor">
                            <input  type="hidden" id="idproveedor1" name="idproveedor1" value="<?php if(isset($idproveedor)){ echo $idproveedor; }else{ if($_SESSION['idproveedor']){ echo $_SESSION['idproveedor'];}} ?> ">
                ...
                <div class="form-row">
                    <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
                        <div class="form-label-group">
                            ...
                            <input type="text" id="prod_nombre" name="prod_nombre" class="form-control" placeholder="Nombre del producto" autocomplete="off" value="<?php if($nombre) echo $nombre; ?>">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="form-row">
                    <div class="col-xs-12 col-sm-3 col-md-3 col-lg-3">
                        <div class="form-label-group">
                            ...
                            <input type="number" id="prod_stockactual" name="prod_stockactual" min="0" class="form-control" autocomplete="off" value="<?php if($stocka) echo $stocka; ?>">
                        </div>
                    </div>
                    <div class="col-xs-12 col-sm-3 col-md-3 col-lg-3">
                        <div class="form-label-group">
                            <label for="prod_stockrecibir"><small>Stock a recibir</small></label>
                            <input type="number" id="prod_stockrecibir" name="prod_stockrecibir" min="0" class="form-control" autocomplete="off" value="<?php if($stockr) echo $stockr; ?>">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="form-row">
                    <div class="col-xs-12 col-sm-3 col-md-3 col-lg-3">
                        <div class="form-label-group">
                            ...
                            <input type="text" id="prod_umedida" name="prod_umedida" class="form-control" placeholder="Unidad de medida del producto" autocomplete="off" value="<?php if($umedida) echo $umedida; ?>">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="table-responsive">
                <table name="tabla" class="table table-bordered col-mb-12">
                    ...
                    <tbody id="tabla">
                    <?php
                    $filas = count($idvencimiento);
                    for($i=0;$i<$filas;$i++){ ?>
                        <tr class="selected" bgcolor="#fffaeb" id="<?php echo $i; ?>" onclick="seleccionar(this.id);">
                            <td id="td_id"><?php echo $i+1; ?></td>
                            <td id="td_id">
                                <input id="vto_lote<?php echo $i; ?>" size="30%" class="form-control" style="color: black" type="number" name="vto_lote<?php echo $i; ?>"  value="<?php echo $vto_lote[$i]; ?>" required="required"/>
                            </td>
                            <td id="td_id">
                                <input id="vto_fecha<?php echo $i; ?>" size="30%" style="color: black" class="form-control" type="date" name="vto_fecha<?php echo $i; ?>" value="<?php echo $vto_fecha[$i]; ?>" required="required"/>
                            </td>
                        </tr>                        
                    <?php } ?>
                    </tbody>
                </table>
                <input type="hidden" id="count_click" name="count_click" value="<?php echo $filas;?>" />
                <input type="hidden" id="filas_creadas" name="filas_creadas" value="<?php echo $filas;?>" />
                <input type="hidden" id="idlocal_prod" name="idlocal_prod" value="<?php echo $idlocal_prod; ?>" />
            </div>
          <br>
          <div class="form-group">
            <div class="form-row">
